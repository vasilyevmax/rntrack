...
---------
 ...
 Уровень 0:  - Ошибки. Выдаются только сообщения о серьёзных ошибках и
               информация по 'Action: Display'.
 Уровень 1:  - Предупреждения. Дополнительно к сообщениям об ошибках выдаются
               предупреждения о несуразностях в обрабатываемых письмах и файлах.
 Уровень 2:  - Информация. Дополнительно к ошибкам и предупреждениям, выдаются
               сообщения о всех действиях, вносящих изменения в обрабатываемые
               объекты.
 Уровень 3:  - Сообщения. Дополнительно к ошибкам, предупреждениям и
               информационным сообщениям, выдаются сообщения о выполняемых
               процедурах, например, о начале сканирования базы.
...
"Bad source address" и установленым флагом Local.
 ...
 В темплейтах можно использовать следующие макросы:
     ...
     @MsgLoops          - Loop Counter (не реализовано).
     ...
     @MaxAge            - Значение 'MaxAge:' (не реализовано).
     @MaxAttachSize     - Значение 'MaxAttachSize:' (не реализовано).
     @Date              - Текущая дата.
...
недостающие кладжи, переводы строк и тому подобное.
 ...
 При работе с аттачами RNtrack воспринимает пробел в поле темы письма как
разделитель между именами файлов. Если приаттаченный файл содержит пробел в
имени, такое имя файла будет вопринято как два имени файла (до пробела и
после). В результате файл не будет обработан.
...
Значение каждого ключа хэша - ссылка на массив. Для получения значения
неповторяющегося кладжа (все кроме Via и SEEN-BY) используйте этот код:

$kludgevalue=$FKludges{"KludgeName"}[0];

Внимание! Вы должны использовать все спецсимволы в имени кладжа (0x01, ":" и
т.д.).
...
Update();		- Обновление текущего сообщения после внесенных
			  скриптом изменений. Вы можете изменить следующие
			  ключи хэша %FMSG:
			  ...
			  По этой причине для изменения файла после "Action:
			  Script" добавляется "Action: Rewrite".
			  ...
			  Action: Rewrite * * * * * *
			  \ Теперь в файле сообщения тема изменена на
                          \ "My new subject!"


ExistsInNodelist("99:99/999");	- Проверка 99:99/999 на наличие в нодлисте

FindHub("99:99/999");           - Возвращает номер хаба для 99:99/999.
                                  Возвращает 0 если хаб не найден.

NewMsg($MsgBase,$FromName,$FromAddr,$ToName,$ToAddr,$Subject,$Flags,$Body)
                                - создать сообщение и положить его в базу

Пример:

Следующая команда

 NewMsg("msgs","Alex","2:5030/1666","User","2:5030/1157.1","msg test","",
        "It is My test");

создаст сообщение (msg) в директории 'msgs' от Alex@2:5030/1666 к
User@2:5030/1157.1 с темой 'msg test' и телом 'It is My test'. Установка любых
флагов и кладжей сейчас не поддерживается, так что значения аргументов $Flags
и $Kludges будут игнорироваться.
...
Несколько примеров вы можете найти в файле test.pl.
 ...
 Данный продукт представляет собой FreeWare, распространяется по принципу
As Is, и авторы никаких гарантий дать не могут. Единственное, авторы обещают
более-менее регулярно исправлять обнаруженные ошибки и выпускать новые версии.
...
* публикуйте в эхоконференции RU.FTRACK
* публикуйте в багтреке на сайте http://sourceforge.net/projects/ftrack-as
* отправляйте разработчикам личной почтой.

Разработчики:
* 2003-2006 Alex Soukhotine
  2:5030/1157@fidonet (ph0enix@users.sourceforge.net)
* 2007-... Stas Degteff (координатор проекта после Алекса)
