 ...
 Указывает, что делать с письмом после выполнения команды Action: Route (или Action: RouteFilebox)
   <Mode> : Always  - После выполнения команды письмо всегда удаляется.
          ...
          : Flag    - Если на письме стоит флаг KillSend (k), то письмо будет
                      удалено, иначе - нет.
 По умолчанию считается, что 'KillRoutedMessage: Always'.
...
--------
 Указывает базу, над которой будет производиться действо. Таких баз
...
маской. Возможные варианты этого слова:
   ...
   Action: Delete                  - Удалить письмо.

   Action: Move "o:\new\dir"       - Переместить письмо в другую базу.
