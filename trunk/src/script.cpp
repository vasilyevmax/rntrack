...
// define __PERL_VERSION__ in makefile with digital number of version without
// point character
...
#define FromSP(i) (char *)SvPV(ST(i), n_a); if(n_a == 0) \
    sp_s = ""
#define XCAL (char *)malloc(n_a + 1)
    ...
    XSdEfInE(perl_Log)
    {
        ...
        {
            Log.Level(LOGE) <<
            " perl_log() argument count (need 1, exist" <<
            (int)items << ")." << EOL;
            abort();
        }
...
//char *sp_s;
    ...
    XSdEfInE(perl_ExistsInNodelist)
    {
        ...
        if(LogLevel >= 5)
        {
            Log.Level(LOGD) << "perl_ExistsInNodelist(\"" << str <<
                               "\") as " << fa << EOL;
        }
...
// NewMsg($MsgBase,$FromName,$FromAddr,$ToName,$ToAddr,$Subject,$Flags,$Kludges,$Body)
    XSdEfInE(perl_NewMsg)
    {
        ...
        if(mb == NULL)
        {
            Log.Level(LOGE) << " perl_NewMsg(): MakeBase(" << base <<
                               ") error" << EOL;
            abort();
        }
        ...
        {
            Log.Level(LOGE) <<
            " perl_NewMsg(): Invalid message base name." << EOL;
            // delete mb;
            mb = NULL;
            abort();
        }
...
int _LoadScriptFile(char * fname)
{
    ...
    if(rc != 0)
    {
        Log.Level(100) << "Error parsing script file '" << fname <<
                          "'." << EOL;
        Log.Level(100) <<
...
#else // if defined (__PERL_VERSION__)
    word = word;
    return SS_ERROR;
#endif /* defined(__PERL_VERSION__) */
} // DoThisWord
...
ScrRet DoThisWordRc(char * word)
{
    ...
    if(LogLevel >= 5)
    {
        Log.Level(LOGD) << "DoThisWordRc(\"" << word << "\") rc == " << i <<
                           EOL;
    }
