...
//        FALSE if the node was in the list as a hub and it is a hub now.
    ...
    while(tr != NULL)
    {
        if((tr->Number & 0xffff) == (Node & 0xffff))
        {
            ...
            else if(((tr->Number & A_MASK) == A_HUB) &&
                    ((Node & A_MASK) == A_HUB))
            {
...
bool FlushSubElements(FILE * fh, Ntr * tmt)
{
    ...
    while(tmt != NULL)
    {
        if(tmt->Number != (unsigned int)-1)
        {
            if(FlushSubElements(fh, tmt->Sub) == FALSE)
            {
                return FALSE;
            }
        }
...
// 1  - Write new _empty_ header.
    ...
    if(fwrite(tmt, sizeof(NdlSign) + sizeof(int) + sizeof(NodeListElem) *
              Lists, 1, fh) != 1)
    {
        Log.Level(LOGE) <<
...
bool NodeLists::LoadOneIndex(FILE * fh, Nch * & Ind)
{
    ...
    for(i = 0; i < tmp; i++)
    {
        if(fread(&tmt[i].Number, sizeof(int), 1, fh) != 1)
        {
            return FALSE;
        }
    }
    ...
    for(i = 0; i < tmp; i++)
    {
        if(LoadOneIndex(fh, Ind[i].Sub) == FALSE)
        {
            return FALSE;
        }
    }
...
bool NodeLists::Load(void)
{
    ...
    if(fseek(fh, sizeof(NdlSign) + sizeof(int) + sizeof(NodeListElem) * Lists,
             SEEK_SET) != 0)
    {
...
// function only with (-1).
    ...
    if(tmt == NULL)
    {
        return (SoftCheckInNodelists == FALSE) ? (unsigned int)-1 : A_HOST;
    }
...
//      Log.Level(LOGD) << "Net " << (Mask.Zone() & 0xffff) << ":" <<
    ...
    {
//      Log.Level(LOGD) << "Node " << Addr << " not equal to mask " << Mask << "
// and mask is not a regional mask." << EOL;
        return FALSE;
    }
