...
int FA::operator ==(const FA & r) const
{
    ...
    if(zone_v & FA_NOTMASK)
    {
        rc = !rc;
    }

    if(r.zone_v & FA_NOTMASK)
    {
        rc = !rc;
    }
...
unsigned int _ParseOneDigit(const char * & p)
{
    ...
    if(*p == NHOLDMASK_CHAR[0])
    {
        p++;
        return FA_HOLDMASK;
    }

    if(*p == NHUBMASK_CHAR[0])
    {
        p++;
        return FA_HUBMASK;
    }

    if(*p == NPVTMASK_CHAR[0])
    {
        p++;
        return FA_PVTMASK;
    }

    if(*p == NDOWNMASK_CHAR[0])
    {
        p++;
        return FA_DOWNMASK;
    }
...
int FA::Parse(const char * & p)
{
    ...
    if(*p == NOTMASK_CHAR[0])   // Found NOT flag. This flag is set _only_ on
    {
        // ZONE
        NotFlag = TRUE;
        p++;
...
// Now, check errors.
    ...
    if(!(zone_v & FA_NOTDEF) && (net_v & FA_NOTDEF))   // zone exists but not a
    {
        // net.
        zone_v  = FA_NOTDEF;
        node_v  = FA_NOTDEF;
    ...
    if(!(net_v & FA_NOTDEF) && (node_v & FA_NOTDEF))   // net exists but not a
    {
        // node.
        zone_v  = FA_NOTDEF;
        net_v   = FA_NOTDEF;
...
char * fatoa(unsigned int i, char * tmp)
{
    ...
    if(i & FA_LSTMASK)
    {
        strcat(tmp, LSTMASK_CHAR);
        return tmp;
    }

    if(i & FA_FROMMASK)
    {
        strcat(tmp, FROMMASK_CHAR);
        return tmp;
    }

    if(i & FA_TOMASK)
    {
        strcat(tmp, TOMASK_CHAR);
        return tmp;
    }

    if(i & FA_OURMASK)
    {
        strcat(tmp, OURMASK_CHAR);
        return tmp;
    }

    if(i & FA_SUBMASK)
    {
        strcat(tmp, SUBMASK_CHAR);
        return tmp;
    }

    if(i & FA_HOLDMASK)
    {
        strcat(tmp, NHOLDMASK_CHAR);
        return tmp;
    }

    if(i & FA_PVTMASK)
    {
        strcat(tmp, NPVTMASK_CHAR);
        return tmp;
    }

    if(i & FA_DOWNMASK)
    {
        strcat(tmp, NDOWNMASK_CHAR);
        return tmp;
    }

    if(i & FA_HUBMASK)
    {
        strcat(tmp, NHUBMASK_CHAR);
        return tmp;
    }
