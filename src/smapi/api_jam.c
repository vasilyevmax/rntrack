                             ...
                             dword len)
{
   ...
   offs=(dword)((byte *)&(SubField->subfield[newcount])-subfield[0]->subfield[0].Buffer);
                                   ...
                                   *subfield, dword clen, unsigned char *ctxt)
{
   ...
   for (ptr=ctxt, len=0; len<=clen; len++, ptr++) {
      if (*ptr=='\0' || len==clen || *ptr=='\001') {
         if (*ctxt && *ctxt!='\001' &&
             StrToSubfield((const unsigned char *)ctxt, (dword)(ptr-ctxt), (dword *)&i, SubField)) {
       ...
       while (*first) {
           ...
           if ((firstlen > 9 && strncmp((char *)first, "SEEN-BY: ", 9) == 0)  ||
               *first == '\001') {
               ...
               assert((dword)(curtext - onlytext) + firstlen +1  <= (dword)textlen);
