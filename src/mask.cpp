...
char fa1[40], fa2[40];
   ...
   if (fMaxAge == 1) strcat(fa1,"+g");
   if (fMaxAge == 2) strcat(fa1,"-g");

   if (fAttExists == 1) strcat(fa1,"+x");
   if (fAttExists == 2) strcat(fa1,"-x");

   if (fPrivate == 1) strcat(fa1,"+p");
   if (fPrivate == 2) strcat(fa1,"-p");

   if (fCrash == 1) strcat(fa1,"+c");
   if (fCrash == 2) strcat(fa1,"-c");

   if (fReceived == 1) strcat(fa1,"+r");
   if (fReceived == 2) strcat(fa1,"-r");

   if (fSend == 1) strcat(fa1,"+s");
   if (fSend == 2) strcat(fa1,"-s");

   if (fFileAttach == 1) strcat(fa1,"+a");
   if (fFileAttach == 2) strcat(fa1,"-a");

   if (fTransit == 1) strcat(fa1,"+i");
   if (fTransit == 2) strcat(fa1,"-i");

   if (fOrphan == 1) strcat(fa1,"+o");
   if (fOrphan == 2) strcat(fa1,"-o");

   if (fKillSend == 1) strcat(fa1,"+k");
   if (fKillSend == 2) strcat(fa1,"-k");

   if (fLocal == 1) strcat(fa1,"+l");
   if (fLocal == 2) strcat(fa1,"-l");

   if (fHold == 1) strcat(fa1,"+h");
   if (fHold == 2) strcat(fa1,"-h");

   if (fFileRequest == 1) strcat(fa1,"+f");
   if (fFileRequest == 2) strcat(fa1,"-f");

   if (fRRQ == 1) strcat(fa1,"+q");
   if (fRRQ == 2) strcat(fa1,"-q");

   if (fIRR == 1) strcat(fa1,"+y");
   if (fIRR == 2) strcat(fa1,"-y");

   if (fARQ == 1) strcat(fa1,"+b");
   if (fARQ == 2) strcat(fa1,"-b");

   if (fFURQ == 1) strcat(fa1,"+u");
   if (fFURQ == 2) strcat(fa1,"-u");

   if (fDIR == 1) strcat(fa1,"+d");
   if (fDIR == 2) strcat(fa1,"-d");

   if (fIMM == 1) strcat(fa1,"+m");
   if (fIMM == 2) strcat(fa1,"-m");

   if (fEmpty == 1) strcat(fa1,"+e");
   if (fEmpty == 2) strcat(fa1,"-e");

   if (fTFS == 1) strcat(fa1,"+t");
   if (fTFS == 2) strcat(fa1,"-t");

   if (fKFS == 1) strcat(fa1,"+j");
   if (fKFS == 2) strcat(fa1,"-j");

   if (fCFM == 1) strcat(fa1,"+n");
   if (fCFM == 2) strcat(fa1,"-n");

   if (fEchomail == 1) strcat(fa1,"+E");
   if (fEchomail == 2) strcat(fa1,"-E");
   ...
   if (fScanned == 1) strcat(fa1,"+v");
   if (fScanned == 2) strcat(fa1,"-v");

   if (fLok == 1) strcat(fa1,"+L");
   if (fLok == 2) strcat(fa1,"-L");

   if (fAS == 1) strcat(fa1,"+S");
   if (fAS == 2) strcat(fa1,"-S");

   if (fLoop != 0) {
      if (fLoop == 1) strcat(fa1,"+");
      if (fLoop == 2) strcat(fa1,"-");
      sprintf(fa2,"%d",Loops);
      strcat(fa1,fa2);
   }
   if (fa1[0] == '\0') strcat(fa1,"*");
   Log << " " << fa1;
}
...
char *tmt2;
   ...
   CHP = 23311;
   cMaxMsgSize = m.Bytes();
   if (fMaxMsg == 1 && (cMaxMsgSize <= sd->MaxMsgSize())) return FALSE;
   if (fMaxMsg == 2 && (cMaxMsgSize > sd->MaxMsgSize())) return FALSE;
   CHP = 23312;
   Log.Level(LOGD) << "Message is equal to mask." << EOL;
   return TRUE;
}
