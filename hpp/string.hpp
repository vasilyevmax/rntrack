 ...
 *
 *  $Id: string.hpp,v 1.3 2005/10/31 19:06:45 ph0enix Exp $
 */
...
#define RSTRLCPY(d,s,l) \
 if (my_strlcpy((d),(s),(l)+1)>(l)) \
  Log.Level(LOGD) << __FILE__ << " (line " << __LINE__ << "): strlcpy() error (src=\'" << (s) << "\' dest=\'" << (d) << "\' length=" << (l) << EOL

#define RSTRLCAT(d,s,l) \
 if (my_strlcat((d),(s),(l)+1)>(l)) \
  Log.Level(LOGD) << __FILE__ << " (line " << __LINE__ << "): strlcat() error (src=\'" << (s) << "\' dest=\'" << (d) << "\' length=" << (l) << EOL
