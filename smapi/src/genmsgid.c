...
char genmsgid_rev[]  = "$Revision: 1.1.1.1 $";
char genmsgid_date[] = "$Date: 2005/01/14 19:22:44 $";
                          ...
                          char ** errstr)
{
        ...
        if(ff == NULL)            /* file not found */
        {
            *pname = '\0';
            ...
            if(try == 0)
            {
                if(direxist(seqpath))
                {
                    goto emptydir;             /* directory exists & empty */
                }
    ...
    emptydir:
        ...
        *pname = '\0';
        ...
        if(errno == ENOENT || errno == EEXIST || ((errno == EPERM || errno == EACCES) && try < 16))
        {
            continue;
        }
